...
router.get('/dashboard', ensureAuthenticated, async function(req, res) 
{
  ...
  const apponanalyst = await LoanApplication.count({where:{application_status:"Approve_And_Sent_To_LRT"}});
  
    LoanApplication.findAll({where:{application_status:"Approve_And_Sent_To_LRT"}}).then((loanapp)=>{
    ...
    i++;
      ...
      if(i>0)
      {
        errors.push({msg:"There Are " + i + " Loan Application On LRT Team Waiting To Update More Than 5 Days"})
        
      }
      console.log(errors);
      res.render('dashboard',{
        user:req.user,appnew:appnew,apponanalyst:apponanalyst,errors,
        success_msg:"There Are "+apponanalyst+" Loan Applications Active On LRT Team"
      })
